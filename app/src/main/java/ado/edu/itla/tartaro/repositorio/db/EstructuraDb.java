...
public class EstructuraDb {
    ...
    public static final String BUSQUEDA_ENTRE_TAREA_USUARIO_C = "SELECT t.*, uc.nombre as usuario_creador, c.nombre as cat_nombre FROM tarea t INNER JOIN usuario uc ON t.usuario_creador_id = uc.id INNER JOIN categoria c ON t.categoria_id = c.id WHERE t.usuario_asignado_id = ? ";
    public static final String BUSQUEDA_ENTRE_TAREA_USUARIO_A = "SELECT t.*, ua.nombre as usuario_asignado, c.nombre as cat_nombre FROM tarea t INNER JOIN usuario ua ON t.usuario_asignado_id = ua.id INNER JOIN categoria c ON t.categoria_id = c.id WHERE t.usuario_creador_id = ? ";
